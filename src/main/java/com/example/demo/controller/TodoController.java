...
//	public ResponseEntity<?> createTodo(@RequestBody TodoDTO dto) {
	public ResponseEntity<?> createTodo(
		...
		try {
			...
			// (3) 임시 사용자 아이디를 설정해 준다. 이 부분은 4 장 인증과 인가에서 수정할 예정이다. 지금은 인증과 인가 기능이 없으므로 한 사용자(temporary-user)만 로그인 없이 사용할 수 있는 애플리케이션인 셈이다.
			//entity.setUserId(temporaryUserId);
			entity.setUserId(userId); // 기존 temporary-user 대신 @AuthenticationPrincipal에서 넘어온 userId로 설정해 준다.
